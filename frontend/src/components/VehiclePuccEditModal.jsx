    ...
    try {
        ...
        Object.keys(payload).forEach(key => {
            if (payload[key] === '' || payload[key] === null) {
                // Keep the required 'valid_until' field, even if temporarily empty during user input
                if (key !== 'valid_until') {
                    delete payload[key];
                }
            }
        ...
        // Use PUT for updates and send the cleaned payload
        await api.put(`/puccs/${puccRecord.id}`, payload);
        toast.success('PUCC record updated successfully.');
        onUpdated?.();
        onHide();
    } catch (err) {
